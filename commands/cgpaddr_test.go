...
func TestParseCGPADDR(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cg.IPAddress != tt.wantIPv4 {
				t.Errorf("CGPADDR.ParseRespone() IPAddress = %q, want %q", cg.IPAddress, tt.wantIPv4)
			}
			if cg.IPv6Address != tt.wantIPv6 {
				t.Errorf("CGPADDR.ParseRespone() IPv6Address = %q, want %q", cg.IPv6Address, tt.wantIPv6)
			}
		})
	}
}
